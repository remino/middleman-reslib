require "spec_helper"
require "middleman-reslib/dashed_dialog_classifier"

RSpec.describe MiddlemanReslib::DashedDialogClassifier do
  ...
  it "adds dashed-dialog to paragraphs with two em-dash dialogue lines" do
    ...
    expect(response.join).to include('<p class="dashed-dialog">')
  end
  ...
  it "uses a custom class name when provided" do
    ...
    middleware = described_class.new(app, class_name: "no-indent")
    allow(app).to receive(:call).and_return([200, headers, [html]])
